...
from advanced_activations import ShiftedELU
from losses import build_gmd_log_likelihood
from callbacks import BatchLossCSVLogger
...
class LungNet(Encoder, NakedModel):
    ...
    def __init__(self,
        ...
        # spp_nb_bins_per_level - Spatial Pyramid Matching or Spatial Pyramid Pooling is a technique that helps us to handle/use multi-scale images efficiently, 
        # especially while dealing with machine learning algorithms to address problems like classification. 
        # It is similar to the Bag of Words. Here is a case where the technique helps in betterment of a Convolutional Neural Network’s (CNN) performance.
        #  A fixed-length output may be generated by the SPP layer(s) regardless of the input size. The multi-level spatial bins used by the SPP layer(s)
        # may provide robustness to object deformations. An SPP layer based system may pool features extracted at variable scales due to the
        # flexibility of input scales making it possible to generate a full-image representation for testing. Moreover, SPP networks may enable feeding of 
        # images with varying sizes or scales during training, which may increase scale-invariance and reduce the risk of over-fitting.
